...
function loadStatesData() {
    fetch('/api/aqi')
        ...
        .then(data => {
            ...
            statesData = Object.keys(data.cities)
            ...
            console.log('Live states data loaded:', statesData);
        })
        .catch(error => {
            console.error('Error loading live states data:', error);
            // Fallback to static data if live data fails
            fetch('map-section/assets/data/states_aqi.json')
                .then(response => response.json())
                .then(data => {
                    statesData = data;
                    console.log('Static states data loaded:', statesData);
                })
                .catch(staticError => {
                    console.error('Error loading static states data:', staticError);
                });
        });
}
