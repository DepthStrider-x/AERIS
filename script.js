...
async function fetchAQIData() {
    try {
        ...
        if (location) {
            ...
            // Default to Muzaffarnagar
            console.log('Available cities:', validCities.map(city => city.city));
            selectedCityData = validCities.find(city => city.city === 'muzaffarnagar') || 
                              validCities.find(city => city.city === 'muzaffar-nagar') ||
                              validCities[0]; // Fallback to first city if Muzaffarnagar not found
            console.log('Default selection:', selectedCityData ? selectedCityData.city : 'None');
        }
...
(function() {
    ...
    function loadWeatherData() {
        ...
        /*
        fetch('sample-forecast.json')
            .then(response => response.json())
            .then(data => {
                // Process the real data
                const forecastData = data.forecast.map(hourData => {
                    const now = new Date();
                    const hour = new Date(now);
                    hour.setHours(now.getHours() + hourData.hour);
                    ...
                    return {
                        ...
                        aqi: hourData.aqi,
                        condition: getWeatherConditionFromText(hourData.condition)
                    };
                });
...
function loadStatesDataForSearch() {
    fetch('map-section/assets/data/states_aqi.json')
        ...
        .then(data => {
            ...
            console.log('States data loaded for search:', data);
        })
        .catch(error => {
            console.error('Error loading states data for search:', error);
        });
}
